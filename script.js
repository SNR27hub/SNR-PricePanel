            ...
            paymentSection = `
                ...
                <div id="utr-form-section" class="card">
                    ...
                    <form id="utr-form">
                        ...
                        <input type="text" id="utr-number" placeholder="UTR / Transaction ID (12 digits)" required>
...
// **FIXED CODE: UTR validation for 12 digits**
const handleUtrSubmission = async (e, projectId) => {
    ...
    if (!clientEmail || !utrNumber) {
        ...
        utrErrorEl.style.color = "var(--danger-color)";
        return;
    }

    // New validation for 12-digit UTR number
    if (utrNumber.length !== 12 || !/^\d+$/.test(utrNumber)) {
        utrErrorEl.textContent = "UTR number must be a 12-digit number.";
        utrErrorEl.style.color = "var(--danger-color)";
        return;
    }
    ...
    utrErrorEl.style.color = "var(--warning-color)";
    utrErrorEl.textContent = "Processing your request...";
...
const handleProjectSave = async (e) => {
    ...
    const clientSetFinalDeveloperPrice = Number(finalDeveloperPriceInput.value);
    ...
    const projectData = {
        ...
        services: services,
        calculatedOriginalMarketTotal: calculatedOriginalMarketTotal,
        marketDiscountPercent: marketDiscountPercent,
        clientDisplayDiscountedMarketPrice: clientDisplayDiscountedMarketPrice,
        clientPayableDeveloperPrice: clientSetFinalDeveloperPrice,
        payments: []
    };
...
const handleAdminProjectActions = async (e) => {
    ...
    if (button.classList.contains('delete-btn')) {
        ...
        if (docSnap.exists()) {
            ...
            populateAdminServices(false, project.services);

            marketDiscountPercentInput.value = project.marketDiscountPercent || '';
            finalDeveloperPriceInput.value = project.clientPayableDeveloperPrice || '';
...
const resetProjectForm = () => {
    ...
    populateAdminServices(true);
    marketDiscountPercentInput.value = '';
    finalDeveloperPriceInput.value = '';
