...
const handleUtrSubmission = async (e, projectId) => {
    ...
    try {
        ...
        const newPaymentRequest = {
            ...
            utr: utrNumber,
            status: 'pending',
            timestamp: new Date() // ***FIXED: Use client-side timestamp instead of serverTimestamp()***
        };

        // --- Firebase Update with arrayUnion ---
        ...
        // Reload client view to show pending status
        loadClientView(projectId); 
        
    } catch (error) {
        console.error("Error submitting UTR:", error);
        // Display a more specific error if possible, but keep it general for client
        utrErrorEl.textContent = `Error submitting UTR. Please try again. (${error.message})`;
        utrErrorEl.style.color = "var(--danger-color)";
    } finally {
...
const handleAdminProjectActions = async (e) => {
    ...
    } else if (button.classList.contains('reject-payment-btn')) { // ***NEW: Handle Reject Button***
        const paymentId = button.dataset.paymentId;
        if (confirm('Are you sure you want to reject this payment request? This will delete the request permanently.')) {
            const projectRef = doc(db, 'projects', id);
            const docSnap = await getDoc(projectRef);
            if (docSnap.exists()) {
                const project = docSnap.data();
                // Filter out the payment to be rejected
        ...
        if (docSnap.exists()) {
            ...
            const updatedPayments = (project.payments || []).map(p => {
                if (p.id === currentVerifyPaymentId) {
                    return { ...p, status: 'verified', verificationTimestamp: new Date(), generatedUsername, generatedPassword };
                }
                return p;
            });
            await updateDoc(projectRef, { payments: updatedPayments });
                ...
                <div class="pending-payments-section">
                    ...
                    ${pendingPayments.map(p => `
                        <div class="payment-request-item">
                            <span>Email: <strong>${p.clientEmail}</strong> | UTR: <strong>${p.utr}</strong></span>
                            <div class="payment-request-actions">
                                ...
                                <button class="reject-payment-btn" data-id="${doc.id}" data-payment-id="${p.id}"><i class="fas fa-times"></i> Reject</button>
                            </div>
                        </div>
                    `).join('')}
